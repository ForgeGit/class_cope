...
request_lock_affli <-'{
   worldData{
       zone(id: %i){
           ...
           journalID,
               name,
               characterRankings(metric:dps, className:"Warlock",specName:"Affliction", page: %i)
               fightRankings(metric:speed, page: %i)
           }
       }
   }
...
request_logs_code <- sprintf(request_lock_affli,1018, range, range)
...
df_cope_affli_ranking <- do.call(bind_rows, output)

write.csv(df_cope_affli_ranking,paste0("df_cope_affli_ranking_",format(Sys.time(), "%Y_%m_%d_h%H_m%M"),".csv"))
```

##### *Quick-Process Rankings (Affliction)

```{r}
df_cope_affli_ranking <- read.csv("response_logs_code_df_COPE_lock2023_09_11_h01_m37.csv")

df_cope_affli_ranking<- df_cope_affli_ranking %>%
  clean_names() %>%
  ...
  mutate(duration_s=duration/1000,
         list_id = factor(list_id,levels=unique(df_cope$List_ID))) %>%
  group_by(list_id) %>%
...
#### -Download log Player (Affliction)

```{r}
logID_unique_cope <- unique(df_cope_affli_ranking$logID)
...
df_cope_affli_logplayers <- do.call(bind_rows, output)

write.csv(df_cope_affli_logplayers,paste0("df_cope_affli_logplayers_",format(Sys.time(), "%Y_%m_%d_h%H_m%M"),".csv"))
```


```{r}
df_cope_affli_master <- df_cope_affli_ranking %>% 
  left_join(df_cope_affli_logplayers, 
            by=c("logID",
                 "character_name"="name",
                 # "server",
                 "class"="subType")) %>%
...
logID_affliplayer_unique_cope <- df_cope_affli_master  %>% 
  group_by(logID,fightID,sourceID) %>%
  summarise(n=n())
```

#### -Download Buffs (Affliction)


```{r}
request_logs_code <- sprintf(request_buffs_str,
                             logID_affliplayer_unique_cope$logID,
                             logID_affliplayer_unique_cope$fightID,
                             logID_affliplayer_unique_cope$sourceID)
...
df_cope_affli_buffs <- do.call(bind_rows, output)

write.csv(df_cope_affli_buffs,paste0("df_cope_affli_buffs",format(Sys.time(), "%Y_%m_%d_h%H_m%M"),".csv"))
...
df_cope_affli_dmg <- do.call(bind_rows, output)
df_cope_affli_dmg <- df_cope_affli_dmg %>% clean_names() %>% select(-c(class_resources))

write.csv(df_cope_affli_dmg,
          paste0("df_cope_affli_dmg",format(Sys.time(), 
                                      "%Y_%m_%d_h%H_m%M"),".csv"))
...
df_cope_affli_casts <- do.call(bind_rows, output)
df_cope_affli_casts <- df_cope_affli_casts %>% clean_names() %>% select(-c(class_resources))

write.csv(df_cope_affli_casts,
          paste0("df_cope_affli_casts",format(Sys.time(), 
                                      "%Y_%m_%d_h%H_m%M"),".csv"))
